...
void read_touch() {
/* Implemented functions on screen: 
void toggle_hold();
void dec_xzoom();
void inc_xzoom();
void dec_yzoom();
...
*/
	...
	if(p.x > TOUCH_X_MIN && p.x < TOUCH_X_MAX && p.y > TOUCH_Y_MIN && p.y < TOUCH_Y_MAX && touch_cnt >= 10) {
		touch_cnt = 0;
...
void show_labels() {
	...
	tft.print("<<");
	tft.setTextColor(TEXT1_COLOUR, BEAM_OFF_COLOUR);
	tft.print(33.0 / y_zoom_factor); //0.33 * 100 / yzoom
	tft.setTextColor(TEXT2_COLOUR, BEAM_OFF_COLOUR);
	tft.print(">>");
	tft.setTextColor(TEXT1_COLOUR, BEAM_OFF_COLOUR);
	tft.setTextSize(1);
